...
if (typeof window.ethereum !== 'undefined') {
    ...
    currentChainId = parseInt(ethereum.chainId, 16);
		if (currentChainId != 1) {
			try {
		        ...
		        chainP.innerHTML = 'Vous utilisez ' + chains[i].name + '. Passez au réseau principal Ethereum pour faire un don.';
		    	}
				}
			} catch {
				chainP.innerHTML = "Assurez-vous d'utiliser le réseau principal Ethereum";
			}
		} else {
			chainP.innerHTML = 'Vous utilisez le réseau principal Ethereum';
		}
	}
  ...
  function getNet() {
	...
	function getExplorerLink(chainId, txHash) {
		...
		if (url) {
			fullstring += '<br>Vous pouvez voir votre transaction <a rel="external" href="'+ url +'"; target=_blank">ici</a></p>'
		}
		return fullstring
	...
	async function getAccount() {
		...
		statusP.innerHTML = "Vous êtes connecté avec ce compte: " + accounts[0];
